...
process PLOT_TFS {
    ...
    publishDir { "${params.proj_dir()}/${species}/10.TFs" },    mode: 'copy',    pattern: "${get_safe(contrast)}/*.pdf"
    ...
    // =================================================================================
    input:
    tuple val(species), val(contrast), path(tf_xlsx)
    // species       : "Human" or "Mouse"
    // contrast      : Contrast label (e.g., "condition_Treated_vs_Control")
    // tf_xlsx      : Pathway results from ANALYZE_PATHWAYS
    ...
    // =================================================================================
    output:
    path("${get_safe(contrast)}/*.pdf"),    emit: tf_plots    // PDF plots for this contrast
    ...
    def LOG           = "${species}_${safe_contrast}_PLOT_TFS.error.log"
    ...
    # Arg 2: tf_xlsx           — path to tf results Excel
    # Arg 3: "."               — output directory (current work dir)
    # Arg 4: metadata_xlsx     — path to sample metadata Excel
    # Arg 5: col_annotation_str— comma-separated metadata columns for heatmap annotations
    # Arg 6: col_cluster_by    — column to use for hierarchical clustering
    # Arg 7: top_n             — number of top pathways to plot
    Rscript "${script_path}/PLOT_TFS.R" \
        "${contrast}" \
        "${tf_xlsx}" \
        "." \
        "${metadata_xlsx}" \
        ...
        "${decoupler.top_n}" > "${LOG}" 2>&1 \
        || { echo "❌ ERROR: PLOT_TFS.R failed for ${species} / ${contrast}" | tee -a "${LOG}" >&2; exit 1; }

    echo "✅ SUCCESS: TF plots generated for ${species} / ${contrast}" >> "${LOG}"
    """
}
