...
plot_deseq2_results <- function(contrast, res, degs, output_dir,
                                vst_nonblind    = NULL,
                                metadata        = NULL,
  ...
  if (!is.null(expr_mat) && !is.null(metadata)) {
    ...
    if (length(sig_genes) >= 2) {
      ...
      if (!is.null(ph)) {
        grDevices::cairo_pdf(filename = file.path(output_path, "Heatmap.pdf"),
                             width    = 10,
                             height   = 11.5,
                             onefile  = TRUE)
  ...
  log_info(sample = "", step = "plot_deseq2_results",
           msg    = glue::glue("DESeq2 plots saved to: '{output_path}'"))
...
if (!interactive()) {
  ...
  plot_deseq2_results(
    ...
    vst_nonblind    = get_arg(5),
    metadata        = get_arg(6),
    col_annotations = col_ann_list,
    col_cluster_by  = get_arg(8, "all")
  )
}
