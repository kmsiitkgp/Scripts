...
process RSEQC {
    ...
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/01_read_distribution" },      mode: 'copy',    pattern: "*.read_distribution*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/02_inner_distance" },         mode: 'copy',    pattern: "*.inner_distance*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/03_junction_annotation" },    mode: 'copy',    pattern: "*.{splice*pdf,junction_summary.txt,junction_plot.r,junction.bed,junction.xls}"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/04_junction_saturation" },    mode: 'copy',    pattern: "*junctionSaturation*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/05_deletion_profile" },       mode: 'copy',    pattern: "*.deletion_profile*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/06_mismatch_profile" },       mode: 'copy',    pattern: "*.mismatch_profile*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/07_insertion_profile" },      mode: 'copy',    pattern: "*.insertion_profile*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/08_clipping_profile" },       mode: 'copy',    pattern: "*.clipping_profile*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/09_gene_body_coverage" },     mode: 'copy',    pattern: "*.geneBodyCoverage*"
    publishDir { "${params.proj_dir()}/${species}/05.RSEQC/07.Logs" },                   mode: 'copy',    pattern: "*.RSEQC.error.log"
