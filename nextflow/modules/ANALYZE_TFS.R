...
analyze_tfs <- function(contrast, species, output_dir, degs = NULL,
  ...
  if (!is.null(degs_df) && !is.null(deg_results$tf)) {
  ...
  output_file <- file.path(output_path, "TF_and_Pathway_Activity.xlsx")
...
if (!interactive()) {
  ...
  analyze_tfs(
    ...
    metadata        = get_arg(4),
    species         = get_arg(5),
    output_dir      = get_arg(6, "."),
    methods         = method_list,
    minsize         = as.numeric(get_arg(8, 5))
  )
}
