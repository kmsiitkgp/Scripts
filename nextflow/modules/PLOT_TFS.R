...
plot_tfs <- function(contrast, tf_xlsx, output_dir,
                     metadata        = NULL,
  ...
  for (analysis_name in names(analysis_list)) {
    ...
    output_file <- file.path(output_path, paste0(type, "_Plot_", analysis_name, ".pdf"))
  ...
  log_info(sample = "", step = "plot_tfs",
           msg    = glue::glue("TF plots saved to: '{output_path}'"))
...
if (!interactive()) {
  ...
  plot_tfs(
    ...
    col_cluster_by  = get_arg(6, "all"),
    n_hits          = as.numeric(get_arg(7, 20))
  )
}
