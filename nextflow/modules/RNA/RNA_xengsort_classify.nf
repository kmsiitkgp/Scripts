...
process RNA_XENGSORT_CLASSIFY {
    ...
    publishDir = [
        //[path: { "${params.proj_dir()}/Human/01.FastQ/xengsort" }, mode: 'copy', pattern: "${sample_id}_graft_R*.gz"],
        //[path: { "${params.proj_dir()}/Mouse/01.FastQ/xengsort" }, mode: 'copy', pattern: "${sample_id}_host_R*.gz"],
        [path: { "${params.proj_dir()}/Xenograft/Logs" },            mode: 'copy', pattern: "*.XENGSORT_CLASSIFY.log"]
    ]
    ...
    path("${sample_id}.XENGSORT_CLASSIFY.log"),          emit: error_log       // Process log
    ...
    def LOG       = "${sample_id}.XENGSORT_CLASSIFY.log"
