...
plot_pathways <- function(contrast, pathway_xlsx,
                          vst_nonblind    = NULL,
                          metadata        = NULL,
  ...
  for (analysis_name in names(analysis_list)) {
    ...
    if (nrow(top_df) == 0) {
      ...
      no_results_file <- file.path(output_dir, paste0("No_Significant_Pathways_Found(", analysis_name, ").pdf"))
    ...
    for (type in c("Bar", "Dot")) {
      ...
      output_file <- file.path(output_dir, paste0(type, "_Plot_Pathways(", analysis_name, ").pdf"))
    ...
    if (length(heatmap_plots) > 0) {
      output_file <- file.path(output_dir, paste0("Heatmap_Plot_Pathways(", analysis_name, ").pdf"))
  ...
  log_info(sample = "", step = "plot_pathways",
           msg    = glue::glue("Pathway plots saved to: '{output_dir}'"))
