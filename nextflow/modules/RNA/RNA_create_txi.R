...
create_txi <- function(species, salmon_dir, output_dir, tx2gene_csv_path = NULL,
  ...
  #
  # Why conditional dropInfReps?
  # If Salmon was run with numBootstraps > 0, bootstrap data is written to
  # aux_info/bootstraps.gz. We detect this automatically and load infReps into
  # the txi object if present — useful for downstream fishpond/tximeta analysis.
  # If absent, dropInfReps = TRUE keeps the txi object lean.

  has_bootstraps <- all(file.exists(
    file.path(dirname(quant_files), "aux_info", "bootstrap", "bootstraps.gz")
  ))

  log_info(sample = "", step = "create_txi",
           msg = glue::glue("Bootstrap data {ifelse(has_bootstraps, 'detected and loaded', 'not found, skipping infReps')}"))

  txi <- tximport::tximport(files           = quant_files,
                            type            = "salmon",
                            tx2gene         = tx2gene,
                            dropInfReps     = !has_bootstraps,
