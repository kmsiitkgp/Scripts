...
#$ -t 1-2                       # Submit an array job with n tasks where n = number of samples = number of fastq files/2 if paired end
...
input=($INPUT_DIR/*.pos.sorted.no.dup.filtered.Y.bam)

# Create an array of filenames with output path
# Create 3 files: unstranded, stranded +, stranded -
read=(${input[*]//$INPUT_DIR/$OUTPUT_DIR})
output1=(${read[*]//.pos.sorted.no.dup.filtered.Y.bam/.txt})
output2=(${read[*]//.pos.sorted.no.dup.filtered.Y.bam/_pos.txt})
output3=(${read[*]//.pos.sorted.no.dup.filtered.Y.bam/_neg.txt})
