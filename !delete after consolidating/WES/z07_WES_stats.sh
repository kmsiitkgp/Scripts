...
BED_DIR=$HOME/projects/WES/S07604514_hg38
REFERENCE_DIR=$HOME/NGSTools/Reference_Genomes/$species

BAIT_BED=$(basename $BED_DIR/*Covered.bed)
TARGET_BED=$(basename $BED_DIR/*Regions.bed)
REFERENCE=$(basename $REFERENCE_DIR/*.fa)

BAIT_LIST=${BAIT_BED//.bed/_list}
TARGET_LIST=${TARGET_BED//.bed/_list}
DICT=${REFERENCE//.fa/.dict}

# Create an array of input files
input=($INPUT_DIR/*pos.sorted.no.dup.filtered.bam)
...
taskoutput=${taskoutput//.pos.sorted.no.dup.filtered.bam/.metrics.txt}
...
# You need to add path for Conda, java
PATH=$HOME/miniconda3/bin/:$PATH
source activate MAGECK
PATH=$HOME/NGSTools/jdk-21.0.2/bin/:$PATH
